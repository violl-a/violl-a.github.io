...
function trackVisitor() {
    const statsRef = db.ref("statistics");

    // إجمالي الزيارات
    statsRef.child("totalVisits").transaction(current => {
        return (current || 0) + 1;
    });
    ...
    statsRef.child("daily").child(today).transaction(current => {
        return (current || 0) + 1;
    });
}
...
// ===== Visitor Analytics (Viola) =====
try {
    const visitDate = new Date().toISOString().slice(0,10);
    const visitRef = firebase.database().ref('statistics/visits/' + visitDate);
    visitRef.transaction(v => (v || 0) + 1);
    firebase.database().ref('statistics/totalVisits').transaction(v => (v || 0) + 1);
} catch(e) {
    console.log('visitor analytics error', e);
}
